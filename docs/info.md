...
## 1. Overview
This codebase is a machine learning pipeline and orchestration system designed to train, evaluate, and compare object detection and instance segmentation models for car parts. It currently supports **YOLO11m-seg**, **Mask R-CNN**, and **Fast R-CNN**, with training submitted to an Azure ML attached GPU VM or run locally. Dataset preparation and pipeline orchestration run entirely in Python via the interactive CLI (`orchestrator.py`). It is designed for ML engineers or researchers working on automotive damage assessment.

## 2. Tech Stack
- **Core Languages**: 100% Python (ML, data preprocessing, and interactive orchestrator CLI). Bash (pipeline runner glue).
- **ML Frameworks**: 
...
- **Utilities**: `openpyxl` (Excel reports), `opencv-python`, `pycocotools`, `supervision`, `imagesize`, `blake3`.

## 3. Folder/File Structure
- `orchestrator.py`: Interactive Python CLI orchestrator that manages pipeline steps, logs them cleanly, and outputs an Excel summary report.
- `scripts/`: Python and Bash scripts holding the actual logic.
  - `data/`: Dataset acquisition and preprocessing (`download_and_prepare_datasets.sh`, `yolo_to_coco.py`, `combine_datasets.py`, `hash_dataset.py`, etc.).
...
- `run_full_pipeline.sh`: A Bash alternative that prompts the user and runs the chosen steps end-to-end.
- `PROJECT_LOG.md` / `README.md`: Historical experimental logs and project instructions.

## 4. How It Works
1. **Entry Point**: The user runs `python orchestrator.py` or `./run_full_pipeline.sh`. Both provide an interactive menu to select tasks (Annotate, Train, Test, Full Pipeline, Azure ML, etc.).
2. **Data Preparation**: If training, scripts fetch external data and merge it with local data in `RAW_DATASET/`. Python scripts convert formats (e.g. YOLO to COCO via header-only reads) and create dataset splits.
3. **Training**: The orchestrator triggers training scripts (e.g. `scripts/training/train_yolo_seg.py`) in sequence based on user selection. It streams output to the terminal and to dedicated log files in `runs_comparison/run_<timestamp>/logs/`.
4. **Evaluation**: Once training completes, `evaluate_confusion_matrix.py` is invoked for each trained model to test on a validation set and compute confusion matrices. 
5. **Output**: Finally, `compare_all_models.py` builds a comparative text table, and `orchestrator.py` aggregates metrics into a formatted Excel report (`car_part_training_results.xlsx`).
...
- **Hardcoded Settings/Paths**: 
  - Project root is anchored to `orchestrator.py`'s directory.
  - Checkpoint paths for evaluation fallback to specific relative paths (`runs_comparison/run_<timestamp>/...`).

## 6. Setup & Usage
**Prerequisites**: Python 3.10+ and PyTorch installed.

**Steps**:
1. Place the project folder in a persistent directory.
