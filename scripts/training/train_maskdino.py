#!/usr/bin/env python3
"""
train_maskdino.py -- MaskDINO Transformer Instance Segmentation Training.
"""
...
def main():
    parser = argparse.ArgumentParser(description="MaskDINO Instance Segmentation Training")
    ...
    run_name = f"maskdino_{ds_path.name if ds_path.is_dir() else args.dataset}"
    logger = UnifiedLogger(os.path.join(args.output_dir, run_name), "maskdino")
    logger.print_dataset_health(str(train_json), str(val_json))

    print(f"[*] Starting MaskDINO Training for {args.epochs} epochs...")
    ...
    print("[OK] MaskDINO training completed successfully!")
