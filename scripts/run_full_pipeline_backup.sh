...
display_menu() {
    ...
    echo "  2) Train Model           - Prepare dataset & train models (YOLO / Mask2Former / Mask R-CNN)"
...
collect_models() {
    ...
    echo "======================================================================"
    echo "  1) YOLO only"
    echo "  2) Mask2Former only"
    echo "  3) Mask R-CNN only"
    echo "  4) YOLO + Mask2Former"
    echo "  5) YOLO + Mask R-CNN"
    echo "  6) Mask2Former + Mask R-CNN"
    echo "  7) All models"
    echo "======================================================================"
...
collect_training_hparams() {
    ...
    echo "  Mask2Former  : epochs=${M2F_EPOCHS}   batch=${M2F_BATCH}   workers=${M2F_WORKERS}"
...
case "$MODEL_CHOICE" in
    ...
    7)
        ...
        if [ "$ENABLE_ONEFORMER" = "1" ]; then
            run_step "04" "train_oneformer" python scripts/training/train_oneformer.py --dataset carparts-seg --epochs 10 --batch 1
        fi

        if [ "$ENABLE_MASKDINO" = "1" ]; then
            run_step "05" "train_maskdino" bash scripts/training/train_maskdino.sh carparts-seg
        fi

        # Step C: Evaluation
        YOLO_WEIGHTS=$(cat last_yolo_weights_path.txt 2>/dev/null || echo "runs_comparison/yolo11m-seg_carparts-seg/weights/best.pt")
        run_step "06a" "evaluate_yolo" python scripts/evaluation/evaluate_confusion_matrix.py --model yolo --weights "$YOLO_WEIGHTS" --dataset carparts-seg
        run_step "06b" "evaluate_mask2former" python scripts/evaluation/evaluate_confusion_matrix.py --model mask2former --weights runs_comparison/mask2former/best_model --dataset carparts-seg
        run_step "06e" "evaluate_maskrcnn" python scripts/evaluation/evaluate_confusion_matrix.py --model maskrcnn --weights runs_comparison/maskrcnn/best_model.pt --dataset carparts-seg
        run_step "07" "compare_all_models" python scripts/evaluation/compare_all_models.py --dataset carparts-seg
        ;;
    ...
    4)
        ...
        run_step "01" "dataset_prep" bash scripts/data/download_and_prepare_datasets.sh
        run_step "02" "train_yolo" python scripts/training/train_yolo_seg.py --model yolo11m-seg --epochs "$YOLO_EPOCHS" --batch "$YOLO_BATCH" --workers "$YOLO_WORKERS" --dataset carparts-seg
        run_step "03" "train_mask2former" python scripts/training/train_mask2former.py --dataset carparts-seg --epochs "$M2F_EPOCHS" --batch "$M2F_BATCH" --num_workers "$M2F_WORKERS"
        run_step "03b" "train_maskrcnn" python scripts/training/train_maskrcnn.py --dataset carparts-seg --epochs "$MRCNN_EPOCHS" --batch "$MRCNN_BATCH" --num_workers "$MRCNN_WORKERS"
